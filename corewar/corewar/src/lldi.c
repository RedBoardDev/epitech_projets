...
void get_lldi_args(int **args, process_t *process, things_t *things)
{
...
void lldi(int **args, op_t op, process_t *process, things_t *things)
{
    ...
    get_lldi_args(args, process, things);
    if (args[0][1] == 1) a = process->reg[args[0][0] - 1];
    if (args[0][1] == 2) a = args[0][0];
    if (args[0][1] == 3) a = get_ind_adress_bytes(args[0][0], things, process);
    if (args[1][1] == 1) b = process->reg[args[1][0] - 1];
    if (args[1][1] == 2) b = args[1][0];
    process->reg[args[2][0] - 1] = get_adress_bytes((a + b), things, process);
    if (a + b == 0)
        process->carry = 1;
    else
        process->carry = 0;
